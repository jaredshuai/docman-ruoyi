...
- 用户现在可以：
  ...
  - 由前端调用 `viewer-url`
  - 跳转 external viewer
  - 通过 backend absolute `src` 读取文档内容
  - 在 TTL 内重复读取
...
- 当前模式：仅 `preview`；`mode=edit`、`saveUrl`、`saveToken` 仅预留，未实现业务回写。
- 本地验证端口：
  - backend：`8080`
  - plus-ui：`3101`
...
- 非阻塞环境现象：若 `plus-ui` 已有健康实例运行在 `3101`，启动脚本可能提示端口占用，但可直接复用现有实例，不影响 viewer preview 验收。
